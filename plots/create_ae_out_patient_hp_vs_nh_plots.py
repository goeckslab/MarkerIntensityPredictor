...
def load_scores() -> pd.DataFrame:
    ...
    for root, dirs, files in os.walk(source_folder):
        for file in files:
            if file == "scores.csv" or files == "hp_scores.csv":
...
if __name__ == '__main__':
    ...
    if args.markers:
        create_violin_plot(data=scores, score=metric, save_folder=save_path,
                           file_name=f"{metric}_ae_denoising_hp_vs_nh_violin",
                           ylim=(0, 0.5))
    else:
        create_violin_plot(data=scores, score=metric, save_folder=save_path,
                           file_name=f"{metric}_ae_denoising_hp_vs_nh_violin_all_markers", ylim=(0, 0.5))
