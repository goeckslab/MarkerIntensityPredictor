...
def create_boxen_plot_by_mode_only(data: pd.DataFrame, metric: str, ylim: List) -> plt.Figure:
    ax = sns.boxenplot(data=data, x="Network", y=metric, hue="Mode", palette={"IP": "lightblue", "EXP": "orange"},
                       order=["EN", "LGBM", "AE", "AE ALL"])
...
if __name__ == '__main__':
    ...
    scores = pd.concat([lgbm_scores, en_scores, ae_scores, ae_all_scores], axis=0)

    # remove column hyper, experiment, Noise, Replace Value
    scores.drop(columns=["HP", "Experiment", "Noise", "Replace Value", "Hyper"], inplace=True)

    # create a new column with the name all. Use the mean of all markers for every row
    print("Scores")
    print(scores)
    input()

    # load image from images fig2 folder
    image = plt.imread(Path("images", "fig3", "auto_encoder_workflow.png"))
    ...
    ax2 = create_boxen_plot(data=ae_scores, metric="MAE", ylim=[0.0, 0.4])

    ax3 = fig.add_subplot(gspec[2, :])
    ax3.text(-0.1, 1.15, "C", transform=ax3.transAxes,
             fontsize=16, fontweight='bold', va='top', ha='right')
    plt.box(False)
    ax3.set_title('AE All', rotation='vertical', x=-0.1, y=0, fontsize=12)
    ax3 = create_boxen_plot(data=ae_all_scores, metric="MAE", ylim=[0.0, 0.4])

    ax4 = fig.add_subplot(gspec[3, :])
    ax3.text(-0.1, 1.15, "D", transform=ax3.transAxes,
             fontsize=16, fontweight='bold', va='top', ha='right')
    plt.box(False)
    ax3.set_title('AE All', rotation='vertical', x=-0.1, y=0, fontsize=12)
    ax3 = create_boxen_plot_by_mode_only(data=scores, metric="MAE", ylim=[0.0, 0.4])
