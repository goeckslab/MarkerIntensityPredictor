...
def calculate_quartile_performance(ground_truth: pd.DataFrame, marker: str, predictions: pd.DataFrame):
...
def calculate_quartile_performance_without_outliers(ground_truth: pd.DataFrame, marker: str, predictions: pd.DataFrame):
    # keep only the rows that are within 3 standard deviations of the mean
    ground_truth = ground_truth[
        np.abs(ground_truth[marker] - ground_truth[marker].mean()) <= (3 * ground_truth[marker].std())].copy()
...
def create_outlier_results_file() -> pd.DataFrame:
    results = []
    for mode in modes:
    ...
    results.to_csv(str(Path(save_path, f"outlier_accuracy.csv")), index=False)

    return results


def create_no_outlier_results_file():
    results = []
    for mode in modes:
        for train_biopsy_name in biopsies:
            if mode == "ip":
            ...
            for marker in markers:
                ...
                for marker_results_dir, _, _ in os.walk(experiment_folder):
                    for experiment_run, _, _ in os.walk(marker_results_dir):
                        ...
                        mae_1, mae_2, mae_3, mae_4, mae_all, quartiles = calculate_quartile_performance_without_outliers(
                            ground_truth=ground_truth,
                            marker=marker,
                            predictions=predictions)
                        # add these values to a dataframe
    ...
    results = pd.concat(results)
    results.to_csv(str(Path(save_path, f"no_outlier_accuracy.csv")), index=False)

    return results


def create_boxen_plot(results: pd.DataFrame, metric: str, title: str):
    # plot the quartiles
...
if __name__ == '__main__':
    ...
    no_outlier_results: pd.DataFrame = create_no_outlier_results_file()
    # change mode to uppercase
    no_outlier_results["Mode"] = no_outlier_results["Mode"].str.upper()

    outlier_results: pd.DataFrame = create_outlier_results_file()
    # change mode to uppercase
    outlier_results["Mode"] = outlier_results["Mode"].str.upper()

    create_boxen_plot(results=outlier_results, metric=metric, title=f"outlier_{metric}")
    create_boxen_plot(results=no_outlier_results, metric=metric, title=f"no_outlier_{metric}")
