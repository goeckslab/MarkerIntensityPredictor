...
if __name__ == '__main__':
    ...
    data = op[(op["Segmentation"] == "Unmicst + S3") & (op["AF Corrected"] == 0)].copy().reset_index(drop=True)
    ...
    plt.tight_layout()
    plt.savefig(f"{save_folder}/op_unmicst_s3_non_af_mae_scores_heatmap.png")

    # Create heatmap for corrected unmicst  + s3
    data = op[(op["Segmentation"] == "Unmicst + S3") & (op["AF Corrected"] == 1)].copy().reset_index(drop=True)
    data.drop(columns=["Segmentation"], inplace=True)
    data = create_heatmap_df(data)
    data = data.pivot(index="Biopsy", columns="Marker", values="Score")
    data = data.loc[[f"{biopsy}" for biopsy in biopsies]]
    ...
    plt.title("Lower values indicate lower errors", x=0.5, fontsize=12)
    plt.tight_layout()
    plt.savefig(f"{save_folder}/op_unmicst_s3_af_mae_scores_heatmap.png")
    ...
    s3_af = op[(op["Segmentation"] == "Unmicst + S3") & (op["AF Corrected"] == 1)].copy().reset_index(drop=True)
    s3_non_af = op[(op["Segmentation"] == "Unmicst + S3") & (op["AF Corrected"] == 0)].copy().reset_index(drop=True)
    ...
    plot_difference_heatmap(data=data,
                            ...
                            suptitle="Out Patient Performance Difference\nMesmer vs. UnMicst + S3 (AF corrected)",
                            ...
                            file_name="op_difference_mesmer_s3_af")
    ...
    plot_difference_heatmap(data=data,
                            ...
                            title="Negative values indicate better performance using UnMICST + S3\nPositive values indicate better performance using Mesmer",
                            file_name="op_difference_mesmer_s3_non_af")
    ...
    plot_difference_heatmap(data=data,
                            ...
                            file_name="op_s3_af_vs_s3_non_af")
    ...
    plot_difference_violin_plot(data=data,
                                title="Out Patient Performance Difference\nSpread between biopsies\nMesmer vs. UnMicst + S3 (AF corrected)",
                                file_name="op_difference_mesmer_s3_af_violin_plot")
    plot_difference_line_plot(data=data,
                              title="Out Patient Performance Difference\nMesmer vs. UnMicst + S3 (AF corrected)",
                              file_name="op_difference_mesmer_vs_s3_non_af_line_plot")
    ...
    plot_difference_violin_plot(data=data,
                                title="Out Patient Performance Difference\nSpread between biopsies\nMesmer vs. UnMicst + S3 (Non AF corrected)",
                                file_name="op_difference_mesmer_s3_non_af_violin_plot")
    plot_difference_line_plot(data=data,
                              title="Out Patient Performance Difference\nMesmer vs. UnMicst + S3 (Non AF corrected)",
                              file_name="op_difference_mesmer_s3_non_af_line_plot")

    # s3_af clustermap
    data = s3_af.copy()
    create_clustermap(data=data, title="Out Patient UnMICST + S3 Segmenter (AF corrected) \nBiopsy vs. Feature",
                      file_name="op_cluster_map_unmicst_s3_af")

    # s3_non_af clustermap
    data = s3_non_af.copy()
    create_clustermap(data=data, title="Out Patient UnMICST + S3 Segmenter (Non AF corrected) \nBiopsy vs. Feature",
                      file_name="op_cluster_map_unmicst_s3_non_af")

    # mesmer clustermap
    data = mesmer.copy()
    create_clustermap(data=data, title="Out Patient Mesmer Segmenter \nBiopsy vs. Feature",
                      file_name="op_cluster_map_mesmer")
