...
if __name__ == '__main__':
    ...
    data = op[(op["Segmentation"] == "Unmicst + S3") & (op["SNR"] == 0)].copy().reset_index(drop=True)
    ...
    for ax in fig.axes:
        for tick in ax.get_yticklabels():
            tick.set_rotation(0)
    plt.xlabel('Feature')
    plt.suptitle("Out Patient MAE Scores\nUnMICST + S3 Segmenter\nNon SNR", x=0.45, fontsize=16)
    plt.title("Lower values indicate lower errors", x=0.5, fontsize=12)
    plt.tight_layout()
    plt.savefig(f"{save_folder}/op_unmicst_s3_non_snr_mae_scores_heatmap.png")

    # Create heatmap for corrected unmicst  + s3
    data = op[(op["Segmentation"] == "Unmicst + S3") & (op["SNR"] == 1)].copy().reset_index(drop=True)
    data.drop(columns=["Segmentation"], inplace=True)
    data = create_heatmap_df(data)
    data = data.pivot(index="Biopsy", columns="Marker", values="Score")
    data = data.loc[[f"{biopsy}" for biopsy in biopsies]]
    ...
    plt.title("Lower values indicate lower errors", x=0.5, fontsize=12)
    plt.tight_layout()
    plt.savefig(f"{save_folder}/op_unmicst_s3_snr_mae_scores_heatmap.png")
    ...
    s3_snr = op[(op["Segmentation"] == "Unmicst + S3") & (op["SNR"] == 1)].copy().reset_index(drop=True)
    s3_non_snr = op[(op["Segmentation"] == "Unmicst + S3") & (op["SNR"] == 0)].copy().reset_index(drop=True)
    ...
    plot_difference_heatmap(data=data,
                            ...
                            file_name="op_difference_mesmer_s3_snr")
    ...
    plot_difference_heatmap(data=data,
                            ...
                            title="Negative values indicate better performance using UnMICST + S3\nPositive values indicate better performance using Mesmer",
                            file_name="op_difference_mesmer_s3_non_snr")
    ...
    plot_difference_heatmap(data=data,
                            ...
                            file_name="op_s3_snr_vs_s3_non_snr")
    ...
    plot_difference_violin_plot(data=data,
                                ...
                                file_name="op_difference_mesmer_s3_snr_violin_plot")
    plot_difference_line_plot(data=data,
                              title="Out Patient Performance Difference\nMesmer vs. UnMicst + S3 (SNR)",
                              file_name="op_difference_mesmer_vs_s3_non_snr_line_plot")
    ...
    plot_difference_violin_plot(data=data,
                                title="Out Patient Performance Difference\nSpread between biopsies\nMesmer vs. UnMicst + S3 (Non SNR)",
                                file_name="op_difference_mesmer_s3_non_snr_violin_plot")
    plot_difference_line_plot(data=data,
                              title="Out Patient Performance Difference\nMesmer vs. UnMicst + S3 (Non SNR)",
                              file_name="op_difference_mesmer_s3_non_snr_line_plot")

    # s3_snr clustermap
    data = s3_snr.copy()
    create_clustermap(data=data, title="Out Patient UnMICST + S3 Segmenter (SNR) \nBiopsy vs. Feature",
                      file_name="op_cluster_map_unmicst_s3_snr")

    # s3_non_snr clustermap
    data = s3_non_snr.copy()
    create_clustermap(data=data, title="Out Patient UnMICST + S3 Segmenter (Non SNR) \nBiopsy vs. Feature",
                      file_name="op_cluster_map_unmicst_s3_non_snr")
