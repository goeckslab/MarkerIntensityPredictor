...
def setup_log_file(save_path: Path):
    save_file = Path(save_path, "random_draw_debug.log")
...
def evaluate_samples(marker: str, train_set: pd.DataFrame, test_set: pd.DataFrame, biopsy: str, train_set_used: bool):
    try:
        ...
        y_hat = train_set.sample(n=1)[marker]
...
if __name__ == '__main__':
    ...
    try:
        ...
        save_path: Path = Path("data", "cleaned_data", "null_model", "random_draw_performance.csv")
        ...
        logging.error(ex)
        biopsy_evaluation = pd.DataFrame(biopsy_evaluation)
        biopsy_evaluation.to_csv(Path("null_model_random_draw_performance.csv"), index=False)
