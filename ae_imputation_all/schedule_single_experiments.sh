...
for biopsy in "${biopsies[@]}"; do
  ...
  do
      echo biopsy="${biopsy}" mode="${mode}" replace_value="${replace_value}" repetitions="${repetitions}"
      ./ae_imputation_all/single_ae_experiment.sh "${biopsy}" "${mode}" "${replace_value}" "${repetitions}"
  done
done