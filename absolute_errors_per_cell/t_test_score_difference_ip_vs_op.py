...
if __name__ == '__main__':
    ...
    en_errors_ip = pd.read_csv(f"data/absolute_error_per_cell/tumor_in_patient_en/{biopsy}.csv")
    en_errors_op = pd.read_csv(f"data/absolute_error_per_cell/tumor_out_patient_en/{biopsy}.csv")
    ...
    for marker in truth.columns:
        ...
        print("EN: ", en_errors_ip[marker].var())
        print("EN: ", en_errors_op[marker].var())
        ...
        results = stats.ttest_ind(en_errors_ip[marker], en_errors_op[marker], equal_var=False)
        print(results)
