...
if __name__ == "__main__":
    ...
    data = sc.read_h5ad(args.x_matrix_file)

    df = pd.DataFrame(data.X, columns=data.var.index)

    if "E-cadherin" in df.columns:
        df.rename(columns={"E-cadherin": "Ecad"}, inplace=True)

    if "Rb" in df.columns:
        df.rename(columns={"Rb": "pRB"}, inplace=True)

    if "ERK-1" in df.columns:
        df.rename(columns={"ERK-1": "pERK"}, inplace=True)
