...
class Predictions:
    ...
    @staticmethod
    def encode_decode_me_vae_data(encoder: Model, decoder: Model, data: list, features: list, save_path: Path = None,
                                  mlflow_directory: str = None, use_mlflow: bool = True) -> Tuple:
        """
        Encodes and decodes the remaining test dataset. Is then further used for evaluation of performance
        """
    ...
    @staticmethod
    def encode_decode_new_me_vae_data(marker_encoder: Model, morph_encoder: Model, decoder: Model,
                                      marker_data: pd.DataFrame, morph_data: pd.DataFrame, features: list,
                                      save_path: Path = None,
                                      mlflow_directory: str = None, use_mlflow: bool = True) -> Tuple:
        """
        Encodes and decodes the remaining test dataset. Is then further used for evaluation of performance
        ...
        _, _, marker_z = marker_encoder.predict(marker_data)
        _, _, morph_z = morph_encoder.predict(morph_data)
        # z = np.concatenate((marker_z, morph_z), axis=1)
